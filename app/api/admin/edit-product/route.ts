...
import { idGenerator } from '@/app/lib/utils/common';
...
export async function POST(req: Request) {
  try {
    ...
    const product = await db.collection('cloth').findOne({ _id: new ObjectId(reqBody.id) });
    ...
    if (reqBody.newImages.length) {
      newImages = reqBody.newImages.map((img: { dataUrl: string; title: string }) => ({
        ...img,
        imgId: idGenerator(),
      }));
    ...
    if (oldImages.length) {
      ...
      const deletedImages = product.img.filter(
    ...
    await db.collection('cloth').updateOne(
        ...
        $set: {
          ...
          img: [
            ...
            ...newImages.map((img: { imgId: string }) => ({
              url: `${process.env.NEXT_PUBLIC_IMAGES_BASE_URL}?id=${img.imgId}`,
              desc: reqBody.name,
            })),
          ],
        },
    ...
    const updatedItem = await db.collection('cloth').findOne({ _id: new ObjectId(reqBody.id) });
