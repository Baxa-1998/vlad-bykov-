...
export const removeProductFromCart = createEvent<{ productId: string; size: string }>();
// 2) Стор корзины
export const $cart = createStore<ICartItem[]>([])
  .on(removeProductFromCart, (state, { productId, size }) =>
    state.filter((i) => !(i.productId === productId && i.size === size)),
  )
  ...
  .on(increaseCount, (state, { productId, size }) =>
    state.map((i) =>
      i.productId === productId && i.size === size ? { ...i, count: i.count + 1 } : i,
    ),
  )
  .on(decreaseCount, (state, { productId, size }) =>
    state
      .map((i) => (i.productId === productId && i.size === size ? { ...i, count: i.count - 1 } : i))
      .filter((i) => i.count > 0),
  );
